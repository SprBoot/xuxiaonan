...
//AJAX向后台请求数据
	$.ajax({
		type:'post',
		url:"${pageContext.request.contextPath}/mform/getbjIformation",
		sync:false,
...
function setHtml(data){
	...
	$(data).each(function(index,object){
		 s += '<h3 class="panel-title">'+object.bjmc+'</h3><p class="panel-subtitle">${object.getDate()}</p>'+object.date+'</p>';
		 s += '<div class="col-md-6"><span class="panel-note"></span></div>';
		 s += '<div class="col-md-6 text-right"><a href="${pageContext.request.contextPath}/mform/ckbjentrance?id='+object.id+'">编辑</a>|<a href="${pageContext.request.contextPath}/mform/deleteTbj?id='+object.id+'">删除</a></div>';
	});
	if(data.length>0){
		$("#bjxs").html(s);
	}else{
