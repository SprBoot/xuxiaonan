...
//AJAX向后台请求数据
	$.ajax({
		type:'post',
		url:"${pageContext.request.contextPath}/mform/getyxIformation",
		sync:false,
...
function setHtml(data){
	...
	$(data).each(function(index,object){
		 s += '<h3 class="panel-title">'+object.yxmc+'"</h3><p class="panel-subtitle">'+object.grgs+'</p>';
		 s += '<div class="col-md-6"><span class="panel-note"></span></div><div class="col-md-6 text-right">';
		 s += '<a href="${pageContext.request.contextPath}/mform/ckyxentrance?id='+object.id+'">编辑</a>|<a href="${pageContext.request.contextPath}/mform/deleteTyx?id='+object.id+'">删除</a></div>';
	});
	if(data.length>0){
		$("#yxxs").html(s);
	}else{
